...
class KVArgsRegisterInfo:
    ...
    # Path-P-B: number of leading dst_kv_ptrs that are c4 host-mirror (DRAM) targets.
    c4_dram_layer_num: int = 0
    # Path-I: number of index-K host-mirror (DRAM) target layers (follow the c4 group).
    index_dram_layer_num: int = 0
    # Path-I SELECTIVE: authoritative per-position DRAM mask (1 byte/position: 1==DRAM mirror,
    # 0==VRAM). Handles the NON-CONTIGUOUS DRAM set (target index-K layers stay VRAM among the
    # offload ones). Empty => fall back to the prefix counts above (plain Path-P-B / non-select).
    kv_dram_mask: list = dataclasses.field(default_factory=list)
    ...
    def from_zmq(cls, msg: List[bytes]):
        return cls(
            ...
            c4_dram_layer_num=(
                int(msg[13].decode("ascii")) if len(msg) > 13 else 0
            ),
            index_dram_layer_num=(
                int(msg[14].decode("ascii")) if len(msg) > 14 else 0
            ),
            kv_dram_mask=(
                list(struct.unpack(f"{len(msg[15])}B", msg[15]))
                if len(msg) > 15 and len(msg[15]) > 0 else []
            ),
        )
...
class TransferStatus:
    ...
    # Path-P-B: c4 history is transferred to the host (DRAM) mirror in a SEPARATE
    # NIXL xfer with its own "_c4" notif. expects_c4 is set by the receiver only in
    # B mode (own_reserve); is_done() then waits for received_c4 too, so decode never
    # starts before the c4 mirror transfer (separate from the VRAM indexer/c128 xfer)
    # has fully landed. Non-B paths leave both False.
    expects_c4: bool = False
    received_c4: bool = False
    # Mark as failed
    ...
    def is_done(self):
        ...
        if self.expects_state and not self.received_state:
            return False
        if self.expects_c4 and not self.received_c4:
            return False
...
class NixlKVManager(CommonKVManager):
    ...
    def register_buffer_to_engine(self):
        # Path-P-B: the first c4_dram_layer_num kv_data_ptrs are c4 buffers redirected
        # to a host (pinned DRAM) mirror; register them as "DRAM". The rest (indexer,
        # c128) stay "VRAM". c4_dram_layer_num=0 (default / prefill side) => all VRAM.
        _c4_dram = getattr(self.kv_args, "c4_dram_layer_num", 0)
        # Path-I: index-K mirror layers follow the c4 group in the buffer list (c4 first,
        # then indexer), so the DRAM prefix covers c4_dram + index_dram leading entries.
        _index_dram = getattr(self.kv_args, "index_dram_layer_num", 0)
        _dram_prefix = _c4_dram + _index_dram
        # Path-I SELECTIVE: an explicit per-position DRAM mask overrides the prefix (the DRAM
        # set is non-contiguous: target index-K layers stay VRAM among the offload ones). When
        # present (len matches kv_data_ptrs) it is authoritative; else fall back to the prefix.
        _mask = getattr(self.kv_args, "kv_dram_mask", None)
        _use_mask = bool(_mask) and len(_mask) == len(self.kv_args.kv_data_ptrs)

        def _is_dram(_i):
            return bool(_mask[_i]) if _use_mask else (_i < _dram_prefix)

        kv_vram_addrs = []
        kv_dram_addrs = []
        for _i, (kv_data_ptr, kv_data_len) in enumerate(
            zip(self.kv_args.kv_data_ptrs, self.kv_args.kv_data_lens)
        ):
            if _is_dram(_i):
                kv_dram_addrs.append((kv_data_ptr, kv_data_len, 0, ""))
            else:
                kv_vram_addrs.append(
                    (kv_data_ptr, kv_data_len, self.kv_args.gpu_id, "")
                )
        self.kv_descs = self.agent.register_memory(kv_vram_addrs, "VRAM")
        logger.debug(f"Register kv tensors, len(kv_addr)= {len(kv_vram_addrs)}")
        if not self.kv_descs:
            raise Exception("NIXL memory registration failed for kv tensors")
        self.kv_dram_descs = None
        if kv_dram_addrs:
            self.kv_dram_descs = self.agent.register_memory(kv_dram_addrs, "DRAM")
            logger.debug(f"Register c4 mirror (DRAM) tensors, len= {len(kv_dram_addrs)}")
            if not self.kv_dram_descs:
                raise Exception("NIXL memory registration failed for c4 mirror tensors")
        aux_addrs = []
    ...
    def send_kvcache(
        ...
        # Path-P-B: the first c4_dram_layer_num entries of layers_params are c4 layers
        # whose dst is the decode-side host (DRAM) mirror; the rest (indexer, c128) are
        # VRAM. A NIXL XferDList is single-mem-type, so c4 (dst DRAM) and the rest (dst
        # VRAM) must be TWO separate xfers. They each carry their own notif ("_c4" vs
        # the original "_kv...") so the receiver waits for BOTH before decode (see
        # TransferStatus.expects_c4). c4_dram_layer_num==0 => original single VRAM xfer.
        c4_src_addrs, c4_dst_addrs = [], []
        vram_src_addrs, vram_dst_addrs = [], []
        # Path-I: index-K layers (grp 1) follow the c4 group (grp 0) in layers_params, and
        # under index-K offload their dst is ALSO a host (DRAM) mirror. Fold them into the
        # SAME DRAM xfer (they share the "_c4" notif; the receiver only needs to know the
        # DRAM transfer completed). DRAM layers = [0, c4_dram + index_dram).
        # Path-I SELECTIVE: when kv_dram_mask is provided (len == #layers_params) it is
        # authoritative per-position (handles the non-contiguous DRAM set: target index-K
        # layers stay VRAM among the offload ones); else fall back to the prefix count.
        _dram_lp = c4_dram_layer_num + index_dram_layer_num
        _use_mask = bool(kv_dram_mask) and len(kv_dram_mask) == len(layers_params)
        for _lp_idx, (src_ptr, dst_ptr, item_len) in enumerate(layers_params):
            is_c4_dram = bool(kv_dram_mask[_lp_idx]) if _use_mask else (_lp_idx < _dram_lp)
            for prefill_index, decode_index in zip(prefill_kv_blocks, dst_kv_blocks):
                src_addr = src_ptr + int(prefill_index[0]) * item_len
                dst_addr = dst_ptr + int(decode_index[0]) * item_len
                length = item_len * len(prefill_index)
                if is_c4_dram:
                    c4_src_addrs.append((src_addr, length, self.kv_args.gpu_id))
                    c4_dst_addrs.append((dst_addr, length, 0))  # DRAM gpu_id ignored
                else:
                    vram_src_addrs.append((src_addr, length, self.kv_args.gpu_id))
                    vram_dst_addrs.append((dst_addr, length, dst_gpu_id))

        logger.debug(
            f"len(src_addrs): before group: {len(prefill_kv_indices)}, "
            f"vram={len(vram_src_addrs)} c4_dram={len(c4_src_addrs)}"
        )
        handles = []
        # non-c4 (indexer/c128, or ALL layers when c4_dram_layer_num==0): VRAM->VRAM.
        if vram_src_addrs:
            src_descs = self.agent.get_xfer_descs(vram_src_addrs, "VRAM")
            dst_descs = self.agent.get_xfer_descs(vram_dst_addrs, "VRAM")
            ...
            handles.append(h)
        # c4 (Path-P-B): src VRAM (P's c4_kv_pool) -> dst DRAM (D's host mirror).
        if c4_src_addrs:
            # notif = "{room}_kv_{chunk}_{islast}_{pp}" -> "{room}_c4_{chunk}_{islast}_{pp}"
            # receiver only checks components[1]=="c4" to set received_c4 (see
            # update_transfer_status); the rest of the string is ignored for c4.
            c4_notif = notif.replace("_kv_", "_c4_", 1)
            src_descs = self.agent.get_xfer_descs(c4_src_addrs, "VRAM")
            dst_descs = self.agent.get_xfer_descs(c4_dst_addrs, "DRAM")
            h = self.agent.initialize_xfer(
                "WRITE", src_descs, dst_descs, peer_name, c4_notif.encode("ascii")
            )
            if not h:
                raise Exception("KVSender failed to create c4 mirror transfer")
            if self.agent.transfer(h) == "ERR":
                raise Exception("KVSender failed to post c4 mirror transfer")
            handles.append(h)
        ...
        for req in reqs_to_be_processed:
            ...
            if self.is_mla_backend or (decode_tp_size == self.attn_tp_size):
                kv_xfer_handle = self.send_kvcache(
                    ...
                    notif,
                    c4_dram_layer_num=getattr(
                        self.decode_kv_args_table[req.agent_name],
                        "c4_dram_layer_num",
                        0,
                    ),
                    index_dram_layer_num=getattr(
                        self.decode_kv_args_table[req.agent_name],
                        "index_dram_layer_num",
                        0,
                    ),
                    kv_dram_mask=getattr(
                        self.decode_kv_args_table[req.agent_name],
                        "kv_dram_mask",
                        [],
                    ),
            ...
            # send_kvcache may return one handle or a list (Path-P-B: 2 xfers).
            if isinstance(kv_xfer_handle, list):
                handles.extend(kv_xfer_handle)
            else:
    ...
    def update_transfer_status(self):
        ...
        for peer_name, messages in notif_map.items():
            ...
            for msg in messages:
                ...
                elif components[1] == "c4":
                    # Path-P-B: c4 host-mirror transfer landed (separate xfer).
                    self.transfer_statuses[room].received_c4 = True
...
class NixlKVReceiver(CommonKVReceiver):
    ...
    def init(
        ...
        # Path-P-B: when c4 is redirected to the host mirror (own_reserve), c4 lands
        # via a SEPARATE NIXL xfer with a "_c4" notif. Wait for it before decode.
        if getattr(self.kv_mgr.kv_args, "c4_dram_layer_num", 0) > 0:
            self.kv_mgr.transfer_statuses[self.bootstrap_room].expects_c4 = True
            ...
            with lock:
                sock.send_multipart(
                    [
                        ...
                        str(
                            getattr(self.kv_mgr.kv_args, "c4_dram_layer_num", 0)
                        ).encode("ascii"),
                        str(
                            getattr(self.kv_mgr.kv_args, "index_dram_layer_num", 0)
                        ).encode("ascii"),
                        # Path-I SELECTIVE: per-position DRAM mask (1 byte each), authoritative
                        # over the prefix counts. Empty bytes when unset (receiver falls back).
                        bytes(
                            bytearray(
                                1 if b else 0
                                for b in getattr(self.kv_mgr.kv_args, "kv_dram_mask", [])
                            )
                        ),
                    ]
                )
