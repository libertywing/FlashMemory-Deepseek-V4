...
class DecodePreallocQueue:
    ...
    def __init__(
        ...
        if self.scheduler.tp_worker.is_hybrid_swa:
            ...
            # 改法A (SGLANG_PD_SWA_WINDOW_PREALLOC=1): _pre_alloc allocates only the
            # page-aligned window tail in the SWA pool, so the swa pool no longer bounds
            # per-request length. Keep the full ceiling (full_max_total_num_tokens) so
            # _check_if_req_exceed_kv_capacity checks against the real limit (~16.78M),
            # not the tiny swa pool. Without the gate, the original min(full, swa) cap
            # is preserved byte-identically.
            self.swa_window_prealloc = (
                os.environ.get("SGLANG_PD_SWA_WINDOW_PREALLOC", "0") == "1"
            )
            if not self.swa_window_prealloc:
                self.max_total_num_tokens = min(
                    self.max_total_num_tokens,
                    self.scheduler.tp_worker.model_runner.swa_max_total_num_tokens,
                )
            else:
                self._validate_swa_window_prealloc()
                # Worst-case SWA pages a single _pre_alloc consumes: the window tail is
                # [align_down(origin_len - window - page), fill_len), so its length is at
                # most (window + page) rounded UP to a page boundary, plus the extra page
                # the align-down can add. Bound it generously: ceil(window/page) + 2 pages.
                # The admission guard must reserve THIS many SWA pages, not a hardcoded 2 —
                # else SWA can have 1 free page (guard passes) while _pre_alloc needs ~3+
                # (alloc_extend_windowed_swa returns None -> assert crash). This is the real
                # per-request SWA concurrency cost and the true concurrency bound.
                _page = self.token_to_kv_pool_allocator.page_size
                _window = self.scheduler.sliding_window_size or 0
                self._swa_pages_per_req = (_window + _page - 1) // _page + 2
        else:
            self.swa_window_prealloc = False

    def _validate_swa_window_prealloc(self):
        """改法A V0 fail-loud guards. Windowed SWA prealloc is only sound for DSv4's
        radix backend (the only live SWA reader is get_swa_page_indices, last-128 only)
        and is incompatible with decode-side offload-kvcache (R6: get_cpu_copy would copy
        dummy-slot-0 data for pre-window positions). Refuse to start otherwise."""
        mr = self.scheduler.tp_worker.model_runner
        backend = getattr(mr, "attn_backend", None)
        backend_name = type(backend).__name__
        # TboAttnBackend wraps the real backend; unwrap if present.
        if backend_name == "TboAttnBackend":
            inner = getattr(backend, "primary", None) or getattr(
                backend, "attn_backend", None
            )
            backend_name = type(inner).__name__ if inner is not None else backend_name
        assert backend_name == "DeepseekV4BackendRadix", (
            f"SGLANG_PD_SWA_WINDOW_PREALLOC requires the DeepseekV4BackendRadix attention "
            f"backend (its only SWA read is the last-{self.scheduler.sliding_window_size} "
            f"window); got {backend_name!r}. Other backends translate the full "
            f"full_to_swa_index_mapping and would read the un-allocated pre-window region."
        )
        assert not self.scheduler.server_args.disaggregation_decode_enable_offload_kvcache, (
            "SGLANG_PD_SWA_WINDOW_PREALLOC is incompatible with "
            "--disaggregation-decode-enable-offload-kvcache (offload copies the whole "
            "sequence's SWA slots incl. the windowed-away pre-window region). Disable one."
        )
        assert hasattr(
            self.token_to_kv_pool_allocator, "alloc_extend_windowed_swa"
        ), (
            "SGLANG_PD_SWA_WINDOW_PREALLOC needs SWATokenToKVPoolAllocator; got "
            f"{type(self.token_to_kv_pool_allocator).__name__} (HiSparse wrapper or "
            "page_size==1 allocator lacks the windowed method). Disable HiSparse."
        )
        page = self.token_to_kv_pool_allocator.page_size
        window = self.scheduler.sliding_window_size
        swa_pool = self.token_to_kv_pool_allocator.swa_available_size()
        per_req = 2 * page  # page-aligned window tail, see _pre_alloc
        logger.info(
            "[改法A] windowed SWA prealloc ENABLED: page=%d window=%s "
            "swa_pool_slots=%d per_req_swa_slots=%d derived_max_concurrency=%d "
            "(full ceiling=%d tokens)",
            page,
            window,
            swa_pool,
            per_req,
    ...
    def _init_kv_manager(self) -> BaseKVManager:
        ...
        if isinstance(self.token_to_kv_pool, DeepSeekV4TokenToKVPool):
            ...
            # Path-P-B: if the swap engine redirected c4 dst to the host mirror (own
            # reserve), tell the KVManager how many leading kv_data_ptrs are c4 so it
            # registers them as DRAM (mirror) instead of VRAM. get_contiguous_buf_infos
            # already returned mirror ptrs for those entries; this gates the mem-type.
            _eng = getattr(self.token_to_kv_pool, "_swap_engine_p", None)
            if _eng is not None and getattr(_eng, "_own_reserve", False):
                kv_args.c4_dram_layer_num = _eng.c4_layer_num
                # Path-I: if index-K is ALSO offloaded, the indexer buffers (grp 1, the
                # next c4_layer_num entries after the c4 group) are ALSO redirected to
                # DRAM mirrors by get_contiguous_buf_infos. Mark them so the NIXL split
                # registers c4+indexer (first 2*c4_layer_num entries) as DRAM.
                if getattr(_eng, "index_k_offload", False):
                    kv_args.index_dram_layer_num = _eng.c4_layer_num
                # Path-I SELECTIVE: the DRAM set within the indexer group is NON-CONTIGUOUS
                # (only offload layers → mirror; target layers stay VRAM). get_contiguous_buf_infos
                # built the authoritative per-position DRAM mask; thread it to the NIXL split so
                # it marks EXACTLY the mirror positions regardless of contiguity. When None/absent
                # (plain Path-P-B or non-selective) the receiver falls back to the prefix counts.
                _mask = getattr(self.token_to_kv_pool, "_kv_dram_mask", None)
                if _mask is not None:
                    kv_args.kv_dram_mask = list(_mask)
    ...
    def resume_retracted_reqs(
        ...
        for i, req in enumerate(self.retracted_queue):
            ...
            # 改法A: full-token budget (allocatable_tokens) reflects the full-attn pool,
            # but _pre_alloc also needs the window TAIL of SWA slots (~ceil(window/page)+2
            # pages, NOT 2). Without the real guard resume proceeds when the full pool has
            # room yet the SWA pool can't fit the tail, so alloc_extend_windowed_swa returns
            # None and _pre_alloc's assert crashes the scheduler. Mirror pop_preallocated.
            if self.swa_window_prealloc:
                page = self.token_to_kv_pool_allocator.page_size
                if (
                    self.token_to_kv_pool_allocator.swa_available_size()
                    < self._swa_pages_per_req * page
                ):
                    break
    ...
    def _update_handshake_waiters(
        ...
        if not self.queue:
            _wedge_trace("handshake.empty", self.tp_rank, 0)
            return

        if all(decode_req.waiting_for_input for decode_req in self.queue):
            _wedge_trace("handshake.allwaiting", self.tp_rank, len(self.queue))
            return

        _wedge_trace("handshake.allreduce", self.tp_rank, len(self.queue))
    ...
    def pop_preallocated(
        ...
        for i, decode_req in enumerate(self.queue):
            ...
            # 改法A: with windowed SWA prealloc the full-token budget above is the length
            # gate, but each request still needs the page-aligned window TAIL of SWA slots
            # (~ceil(window/page)+2 pages), not just 2. Guard the swa pool for the REAL
            # per-req cost — this is the true concurrency bound (swa_pool / per_req_pages).
            if self.swa_window_prealloc:
                page = self.token_to_kv_pool_allocator.page_size
                if (
                    self.token_to_kv_pool_allocator.swa_available_size()
    ...
    def _pre_alloc(self, req: Req) -> torch.Tensor:
        ...
        elif self.swa_window_prealloc:
            # 改法A: allocate the full sequence in the (byte-free) full allocator, but
            # only the page-aligned window tail in the SWA pool. The decode read +  PD
            # transfer paths both touch only the last `sliding_window` positions, so the
            # pre-window SWA slots were never read/written — windowing them away is the
            # entire concurrency win (per-req SWA footprint drops ~fill_len/window).
            page = self.token_to_kv_pool_allocator.page_size
            window = self.scheduler.sliding_window_size
            # Base win_start on origin_input_ids (the same basis the transfer path uses,
            # line ~562-566) MINUS one extra page of slack, page-aligned down. This
            # guarantees [win_start, fill_len) is a superset of both the transfer write
            # region [align_down(origin-window), origin) and the decode read region
            # (last `window` of fill_len), regardless of how many output_ids prealloc sees.
            origin_len = len(req.origin_input_ids)
            win_start = max(0, origin_len - window - page)
            win_start = (win_start // page) * page
            kv_loc = self.token_to_kv_pool_allocator.alloc_extend_windowed_swa(
                full_seq_len=fill_len,
                swa_win_start=win_start,
                device=self.token_to_kv_pool_allocator.device,
            )
            # tell _evict_swa the pre-window region is already absent from the SWA pool
            req.swa_evicted_seqlen = win_start
        else:
            device = self.token_to_kv_pool_allocator.device
...
class DecodeTransferQueue:
    ...
    def _commit_transfer_to_req(self, decode_req: DecodeRequest) -> None:
        ...
        # ── Path-P PD hook: make the request's c4 history available to decode recall.
        # Two modes:
        #  * B (own_reserve, NIXL c4-direct-to-mirror): the c4 history ALREADY landed
        #    in the CPU mirror via the separate "_c4" DRAM xfer. Just drop those pages
        #    from `_copied` so the first decode recall re-materializes them into the
        #    reserve. NO GPU->mirror copy (the GPU c4 pool was never the c4 dst).
        #  * legacy (shared pool): the full c4 history landed in the GPU c4 pool; copy
        #    it into the mirror (ingest_transferred_history).
        # Runs once per request before it enters the decode run-queue. No-op w/o engine.
        try:
            _kvcache = self.scheduler.token_to_kv_pool_allocator.get_kvcache()
            _eng = getattr(_kvcache, "_swap_engine_p", None)
            if _eng is not None:
                _seq = len(decode_req.req.origin_input_ids)
                _full_locs = self.scheduler.req_to_token_pool.req_to_token[
                    decode_req.req.req_pool_idx, :_seq
                ]
                _logical_pages = torch.unique(_full_locs // 4 // _eng.page_size_chunks())
                if getattr(_eng, "_own_reserve", False):
                    ...
                    logger.info(
                        f"[Path-P PD] ingested rid={decode_req.req.rid} seq={_seq} "
                        f"-> {_n} c4 pages into CPU mirror "
                        f"(page range [{int(_logical_pages.min())},{int(_logical_pages.max())}] "
                        f"full_loc max={int(_full_locs.max())})"
                    )
        except Exception as e:
            logger.error(f"[Path-P PD] c4 mirror hook failed: {e}")
    ...
    def pop_transferred(self, rids_to_check: Optional[List[str]] = None) -> List[Req]:
        if not self.queue:
            _wedge_trace("transfer.empty", self.tp_rank, 0)
            return []
        _wedge_trace("transfer.allreduce", self.tp_rank, len(self.queue))
        polls = poll_and_all_reduce(
            [decode_req.kv_receiver for decode_req in self.queue], self.gloo_group
        )
