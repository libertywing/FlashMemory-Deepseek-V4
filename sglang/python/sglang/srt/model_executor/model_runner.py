...
from sglang.srt.layers.attention.compressed.resident_mask_capturer import (
    ResidentMaskCapturer,
    get_resident_mask_capturer,
    pathp_cudagraph_enabled,
    set_resident_mask_capturer,
)
...
class ModelRunner(ModelRunnerKVCacheMixin):
    ...
    def __init__(
        ...
        # Lightning Indexer decode-swap (Stage 3b): reuse HiSparse pool+allocator
        # (layout-correct small device pool + index mapping that alloc_extend auto-
        # populates for prefilled tokens) but NOT the HiSparse coordinator. Our own
        # code drives map_last_loc_to_buffer (new tokens) + swap scheduling.
        import os as _os_ds
        self.enable_decode_swap = _os_ds.environ.get("SGLANG_DECODE_SWAP", "0") == "1"
    ...
    def initialize(self):
        ...
        self.init_resident_mask_capturer()

        if self.device == "cuda":
            self.init_cublas()
            self.init_attention_backend()
            self.kernel_warmup()
            if self.enable_hisparse and not self.enable_decode_swap:
            ...
            # (A) Pre-build score-resident fixed-address buffers BEFORE graph capture, so the
            # captured decode graph bakes the score-resident (+ fused-remap) branch instead of
            # full-history+mask. Without this, capture runs with resident_kv_scratch=None →
            # _score_resident_active=False frozen → score-resident is dead under cuda graph
            # (proven 2026-06-29 via CAP-PROBE). No-op when score-resident is off.
            _rmc_pre = get_resident_mask_capturer()
            if _rmc_pre is not None and getattr(self, "token_to_kv_pool", None) is not None:
                try:
                    _rmc_pre.prebuild_for_capture(self.token_to_kv_pool)
                except Exception as _e:
                    logger.warning(
                        f"[ResidentMask] prebuild_for_capture failed (graph will bake "
                        f"full-history path): {_e}"
                    )
    ...
    def init_resident_mask_capturer(self):
        """Phase-1 cuda-graph-compatible two-level recall: create the global static
        resident_chunk_mask buffer the captured indexer graph reads every step. Only
        when SGLANG_PATHP_CUDAGRAPH=1 AND the pool is DSv4 (has c4_logical_size).
        Otherwise leave the global capturer None (indexer falls back to native /
        eager hook)."""
        if not pathp_cudagraph_enabled():
            set_resident_mask_capturer(None)
            return
        pool = getattr(self, "token_to_kv_pool", None)
        c4_logical_size = getattr(pool, "c4_logical_size", None)
        if c4_logical_size is None:
            logger.warning(
                "[ResidentMask] SGLANG_PATHP_CUDAGRAPH=1 but pool has no "
                "c4_logical_size (not DSv4?) — disabling graph-compat mask path."
            )
            set_resident_mask_capturer(None)
            return
        interval = int(os.environ.get("SGLANG_RETRIEVER_INTERVAL", "64"))
        hidden_dim = getattr(self.model_config, "hidden_size", 4096)
        set_resident_mask_capturer(
            ResidentMaskCapturer(
                c4_logical_size=c4_logical_size,
    ...
    def forward(
        ...
        # Phase-2 CHURN-CRASH FIX (2026-06-29): reset score-resident per-row buffers for decode
        # rows whose occupant changed (filter_batch compaction / slot reuse) BEFORE the forward's
        # captured indexer reads them. On the D PD-decode server the prefill-side reset never
        # fires (pure decode), so this is the only cross-request cleanup. Out of graph, ours-only.
        _rmc_begin = get_resident_mask_capturer()
        if _rmc_begin is not None:
            _rmc_begin.on_forward_begin(
        ...
        # Phase-1 Strategy A: side-band Level-1 Memory Indexer scoring (cycle boundary)
        # → finalize resident_set → scatter into the global resident_chunk_mask the
        # captured indexer graph reads next window. Host syncs legal here (out of graph).
