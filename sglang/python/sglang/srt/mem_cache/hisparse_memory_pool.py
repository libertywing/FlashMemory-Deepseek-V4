...
class HiSparseTokenToKVPoolAllocator(BaseTokenToKVPoolAllocator):
    ...
    def translate_loc_from_full_to_swa(self, kv_indices: torch.Tensor):
        # PD-disaggregation decode path (disaggregation/decode.py:565) +
        # attention backends call this on the allocator. HiSparse wraps the
        # logical (SWA) allocator, so delegate SWA translation to it — same
        # pattern as swa_available_size / free_swa above.
        return self.logical_attn_allocator.translate_loc_from_full_to_swa(kv_indices)

    def get_kvcache(self):
        # Some disagg/attention paths fetch the underlying kvcache via the
        # allocator; forward to the wrapped logical allocator's kvcache.
        return self.logical_attn_allocator.get_kvcache()
