...
class ScheduleBatch(ScheduleBatchDisaggregationDecodeMixin):
    ...
    def release_req(self, idx: int, remaing_req_count: int, server_args: ServerArgs):
        ...
        if server_args.disaggregation_mode == "decode":
            # Path-P / DeepSeek-V4 hybrid pool does NOT implement get_cpu_copy (offload),
            # so the original unconditional offload raised NotImplementedError here and
            # crashed the whole scheduler (SIGQUIT) the moment decode memory pressure forced
            # a retraction at high concurrency. Guard it: if the pool can't offload, degrade
            # to plain release below (the KV is still freed, the retracted request is simply
            # preempted and re-fetched from P) instead of taking down the scheduler.
            try:
                req.offload_kv_cache(
                    self.req_to_token_pool, self.token_to_kv_pool_allocator
                )
            except NotImplementedError:
                if not getattr(server_args, "_warned_no_offload", False):
                    server_args._warned_no_offload = True
                    logger.warning(
                        "[retract] KV offload-to-CPU not implemented for this pool; "
                        "retracting by plain release (request re-fetched from prefill). "
                        "Avoids the scheduler crash under decode memory pressure."
                    )
    ...
    def get_model_worker_batch(
        ...
        # 更新 req_pool_idx → rid 全局映射表（供 deepseek_v4.py hook 使用）
        try:
            from sglang.srt.models import deepseek_v4 as _dsv4
            _dsv4._req_pool_to_rid = {
                self.req_pool_indices[i].item(): self.reqs[i].rid
                for i in range(len(self.reqs))
            }
        except Exception:
            pass
...
class ModelWorkerBatch:
    ...
    # For request ID tracking (供 deepseek_v4.py hook 使用)
