...
class SchedulerRuntimeCheckerMixin:
    ...
    def check_memory(self: Scheduler):
        if self.enable_hisparse:
            return
        # Decode-swap (Stage 3b) uses the HiSparse allocator (without coordinator),
        # whose available-size accounting differs from the plain checker → skip the
        # leak detector here, same as the hisparse path above.
        import os as _os_ds
        if _os_ds.environ.get("SGLANG_DECODE_SWAP", "0") == "1":
            return
