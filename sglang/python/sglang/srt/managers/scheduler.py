...
class Scheduler(
    ...
    def __init__(
        ...
        # Decode-swap (Stage 3b) reuses HiSparse pool but NOT the coordinator, so
        # the scheduler's hisparse-coordinator branches must stay off for it.
        import os as _os_ds
        self.enable_hisparse = server_args.enable_hisparse and not (
            _os_ds.environ.get("SGLANG_DECODE_SWAP", "0") == "1"
        )
