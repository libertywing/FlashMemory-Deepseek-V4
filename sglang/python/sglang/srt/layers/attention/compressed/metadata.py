...
class PagedIndexerMetadata(IndexerMetadata):
    ...
    def __post_init__(self):
        # ── EXPERIMENT (env SGLANG_INDEXER_SCORE_CAP_CHUNKS): cap the per-step indexer
        # scoring window to a FIXED number of c4 chunks, so decode scoring FLOP becomes
        # O(cap) instead of O(full_context). c4_seq_lens is in chunk units (token//4),
        # so cap=4096 ≈ last 16K tokens. We clamp BEFORE deriving deep_gemm_metadata /
        # topk_metadata so the kernel, its schedule, and topk all stay consistent.
        # NOTE: this scores the FIRST `cap` chunks (positions [0,cap)); same FLOP as a
        # last-N window, so it is a valid THROUGHPUT probe (not a correctness/needle run).
        # 0/unset = no cap = exact original behavior.
        _score_cap = int(os.environ.get("SGLANG_INDEXER_SCORE_CAP_CHUNKS", "0"))
        if _score_cap > 0:
            self.c4_seq_lens = self.c4_seq_lens.clamp(max=_score_cap)
