    ...
    def forward(
        ...
        **_,
        ...
        if isinstance(core_attn_metadata, DSV4AttnMetadataRadix):
            ...
            if compress_ratio == 4:
                # ── Path-P recall + read-remap (HiSparse-free) ──────────────────
                # The topk left FULL-pool compressed locs in c4_sparse_page_indices.
                # Path P keeps c4 history on a CPU mirror + a SHRUNK GPU reserved
                # region. Runs here (after this layer's c4 store, before attention)
                # so the mirror is populated. recall_pages append-only swaps in the
                # touched pages for ALL c4 layers; remap rewrites each selected loc
                # to its reserved-region slot loc (non-resident -> -1 = masked).
                _eng = getattr(token_to_kv_pool, "_swap_engine_p", None)
                if _eng is not None:
                    # BOTH prefill and decode sparse-read c4 history. Chunked
                    # prefill (chunk 2+ of a long request) attends the compressed
                    # c4 of earlier chunks, which Path-P stored to the CPU mirror
                    # and left absent from the shrunk GPU pool. So recall+remap
                    # must run here in prefill too; remap is now vectorized (the
                    # original hang was a Python scalar loop, since fixed). The
                    # compressor store for the current chunk ran before this, so
                    # the mirror is populated. recall_pages append-only swaps in
                    # the touched pages; remap rewrites each selected full-pool
                    # loc to its reserved-region slot loc (non-resident -> -1).
                    _clid = token_to_kv_pool.layer_mapping[layer_id].compress_layer_id
                    _sel = core_attn_metadata.c4_sparse_page_indices
                    # _sel (native top-512) is already confined to the resident_set by the
                    # two-level masking (resident-excluded chunk logits are -inf).
                    #
                    # Phase-2 cuda-graph: in DECODE the recall is moved to the cycle-boundary
                    # side-band (resident_mask_capturer.on_forward_end), which swaps the WHOLE
                    # resident_set into the reserve once per 64-step window — so here in decode
                    # we must NOT call recall_chunks (it host-syncs → forbidden under graph
                    # capture). We only remap, and IN-PLACE (.copy_) so the captured fixed
                    # address of c4_sparse_page_indices is preserved across replays. The
                    # invariant _sel ⊆ resident_set ⊆ reserve (side-band recalled the whole
                    # resident_set) guarantees remap hits ~100%.
                    #
                    # PREFILL (chunked prefill chunk 2+) is NOT under graph capture and the
                    # side-band only fires in decode, so prefill keeps the original
                    # recall+remap (swap in what this chunk sparse-reads, then remap).
                    ...
                    _phase2_decode = (
                        _rmc is not None and forward_batch.forward_mode.is_decode()
                    )
                    if _phase2_decode:
                        # decode: side-band already recalled the resident_set; remap in-place.
                        # FUSED-REMAP (SGLANG_PATHP_FUSED_REMAP=1): when the indexer took the
                        # score-resident path it ALREADY wrote RESERVE CELLS into
                        # c4_sparse_page_indices (via col_to_cell_buf), so remap_compressed_locs
                        # ×21 is redundant — skip it (the run=1 lever; remap×21 was the offload-
                        # only per-step cost, eager-UB 1.43ms, that ate the topk saving).
                        # CRITICAL: skip ONLY when score-resident was ACTUALLY active for this
                        # layer this step. It falls back to FULL-HISTORY (writing PHYSICAL pool
                        # locs, not cells) when resident_count==0 (a fresh req's first steps
                        # before its step-0 finalize) or scratch not built — feeding physical
                        # locs straight to attention as if cells = OOB illegal access. So gate
                        # on the SAME condition the indexer used (_score_resident_active).
                        _fused = (
                            os.environ.get("SGLANG_PATHP_FUSED_REMAP", "0") == "1"
                            ...
                            and getattr(_rmc, "col_to_cell_buf", None) is not None
                        )
                        if not _fused:
                            core_attn_metadata.c4_sparse_page_indices.copy_(
                                _eng.remap_compressed_locs(_clid, _sel)
                            )
                    else:
                        # prefill (or eager non-graph Path-P): recall what this step reads,
                        # then remap (rebind is fine off the graph capture path).
                        _eng.recall_chunks(_clid, _sel.reshape(-1))
