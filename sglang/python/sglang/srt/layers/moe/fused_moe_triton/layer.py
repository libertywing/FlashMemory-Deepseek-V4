...
class FusedMoE(torch.nn.Module):
    ...
    def _load_w13(
        ...
        # 1. CPU (always)
        # 2. GPU with padded intermediate_size_per_partition
        # This handles the case where the loaded weights are smaller than the padded expert_data
        use_padded_loading = _is_cpu
        if not use_padded_loading and not self.use_presharded_weights:
            loaded_shard_size = loaded_weight.shape[shard_dim] // self.moe_tp_size
            use_padded_loading = loaded_shard_size < shard_size
