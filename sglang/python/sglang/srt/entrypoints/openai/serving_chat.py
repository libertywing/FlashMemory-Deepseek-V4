...
class OpenAIServingChat(OpenAIServingBase):
    ...
    def _apply_jinja_template(
        ...
        if self.chat_encoding_spec is not None:
            ...
            if 'MAX_INPUT_TOKEN_LEN' in os.environ and os.getenv('MAX_INPUT_TOKEN_LEN') is not None and os.getenv('MAX_INPUT_TOKEN_LEN').strip().isnumeric():
                max_prompt_len = int(os.getenv('MAX_INPUT_TOKEN_LEN'))
                system_prompt_len = 1
                if 'SYSTEM_PROMPT_TOKEN_LEN' in os.environ and os.getenv('SYSTEM_PROMPT_TOKEN_LEN') is not None and os.getenv('SYSTEM_PROMPT_TOKEN_LEN').strip().isnumeric():
                    system_prompt_len = int(os.getenv('SYSTEM_PROMPT_TOKEN_LEN')) - 16
                token_num = len(prompt_ids)
                if max_prompt_len is not None and token_num > max_prompt_len:
                    logger.warning(
                        "The maximum prompt length is %d tokens. However, "
                        "there are %d tokens in this request. "
                        "And it will be truncated.",
                        max_prompt_len,
                        token_num,
                    )
                    prompt_ids = (
                        prompt_ids[0:system_prompt_len]
                        + prompt_ids[
                            token_num - max_prompt_len + system_prompt_len :
                        ]
                    )
