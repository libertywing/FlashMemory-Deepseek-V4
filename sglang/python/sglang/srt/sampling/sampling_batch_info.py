...
class SamplingBatchInfo:
    ...
    def from_schedule_batch(cls, batch: ScheduleBatch, vocab_size: int):
        ...
        # ── PATCH (Lightning-Indexer): always populate custom_params ────────
        # Upstream only populates `custom_params` when at least one request
        # carries a `custom_logit_processor`. We need it for per-request
        # retriever routing (see retriever_hook_impl.MultiRetrieverHook),
        # which has nothing to do with custom logit processors. Populate the
        # list whenever ANY request carries a non-None custom_params dict.
        if any(r.sampling_params.custom_params is not None for r in reqs):
    ...
    def filter_batch(self, keep_indices: List[int], keep_indices_device: torch.Tensor):
        ...
        if self.has_custom_logit_processor:
            ...
            # ── PATCH (Lightning-Indexer): filter custom_params even when
            # there is no custom logit processor (e.g. per-request retriever
            # routing via MultiRetrieverHook). Without this, custom_params
            # length would diverge from the rest of the batch on retract.
    ...
    def merge_batch(self, other: "SamplingBatchInfo"):
        ...
        # ── PATCH (Lightning-Indexer): merge custom_params even when neither
        # side has a custom logit processor (per-request retriever routing).
        if (
            (self.custom_params is not None or other.custom_params is not None)
            and not (self.has_custom_logit_processor or other.has_custom_logit_processor)
        ):
            self.custom_params = self.custom_params or [None] * len(self)
            other_cp = other.custom_params or [None] * len(other)
            self.custom_params.extend(other_cp)
